...
export const checkExpirationsAndNotify = async () => {
  ...
  try {
    ...
    for (let leader of leaders) {
      ...
      if (
        leader.expirationDate &&
        moment(leader.expirationDate).isBetween(today, oneWeekAhead)
      ) {
        notifications.push({
          title: "License Expiration Reminder",
          description: `Your license will expire on ${moment(
            leader.expirationDate
          ).format("YYYY-MM-DD")}. Please update it.`,
          receiver: leader._id,
      ...
      if (
        leader.IDExpireDate &&
        moment(leader.IDExpireDate).isBetween(today, oneWeekAhead)
      ) {
        notifications.push({
          title: "ID Expiration Reminder",
          description: `Your ID will expire on ${moment(
            leader.IDExpireDate
          ).format("YYYY-MM-DD")}. Please update it.`,
          receiver: leader._id,
          receiverModel: "TripLeader",
    ...
    for (let tool of tools) {
      await notificationModel.create({
        ...
        description: `The license for tool ${tool.name} will expire on ${moment(
          tool.licenseEndDate
        ).format("YYYY-MM-DD")}. Please renew it.`,
    ...
    for (let place of places) {
      await notificationModel.create({
        ...
        description: `The license for place ${
          place.name
        } will expire on ${moment(place.ExpiryDate).format(
          "YYYY-MM-DD"
        )}. Please renew it.`,
